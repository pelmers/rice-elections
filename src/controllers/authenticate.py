...
class LoginResponseHandler(webapp2.RequestHandler):
    ...
    def get(self):
        ...
        if not ticket:
            return render_template('/templates/message', {'status': 'Error', 'msg': 'Ticket not specified.'})

        net_id = self.validate_cas_2()
        if not net_id:
            return render_template('/templates/message', {'status': 'Error', 'msg': 'Invalid ticket. Credentials not verified.'})
        ...
        if not destination_url:
            return render_template('/templates/message', {'status': 'Error', 'msg': 'User authenticated. However, no destination '
                              'url is provided.'})
...
class LogoutResponseHandler(webapp2.RequestHandler):
    def get(self):
        ...
        if session.has_key('net_id'):
            session.terminate()
            return render_template('/templates/message', {'status': 'Logged Out', 'msg': 'You\'ve been logged out. See you again soon!'})
        else:
            return render_template('/templates/message', {'status': 'Silly', 'msg': 'You weren\'t logged in.'})
